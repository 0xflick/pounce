...
use flichess::board::{Board, ParseFenError};
use flichess::search::Search;
use flichess::table::Table;
...
fn main() {
    ...
    for test in TEST_SUITE.lines() {
        ...
        let fen_parts: Vec<&str> = parts.by_ref().take(4).collect();
        let fen = fen_parts.join(" ") + " 0 1";

        if let Some(operations) = parts.next() {
        ...
        if let (fen, best_moves, Some(name)) = (fen, best_moves, name) {
            tests.push(Test {
                fen,
                best_moves,
                name,
    ...
    println!(
        ...
        total,
        num_sucesses.to_string().green(),
        (total - num_sucesses).to_string().red()
    );
}
